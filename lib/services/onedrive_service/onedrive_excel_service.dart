...
class OneDriveExcelService {
  ...
  Future<OneDriveFileRef> resolveShareLink(String shareUrl, String accessToken) async {
    ...
    if (response.statusCode != 200) {
      throw OneDriveExcelException(
        'Excel dosyasına ulaşılamadı. Lütfen şunları kontrol edin:\n'
        '1. Bağlantının bir "Paylaşım" bağlantısı olduğundan (tarayıcıdaki adres çubuğu değil).\n'
        '2. Bağlantı izninin "Düzenleyebilir" olduğundan.\n'
    ...
    // If it's a shared item from another drive, drive info is in remoteItem
    final remoteItem = json['remoteItem'] as Map<String, dynamic>?;
    
    final driveId = (remoteItem?['parentReference']?['driveId'] ?? json['parentReference']?['driveId']) as String?;
    final itemId = (remoteItem?['id'] ?? json['id']) as String?;
    final name = (remoteItem?['name'] ?? json['name']) as String?;

    if (driveId == null || itemId == null) {
      throw OneDriveExcelException('Dosya kimlik bilgileri (driveId/itemId) çözülemedi.');
    }
    
    return OneDriveFileRef(driveId: driveId, itemId: itemId, name: name);
  }
  ...
  /// Returns the webUrl of the modified file.
  Future<String?> exportReceipt(OneDriveFileRef ref, ReceiptData data, String accessToken) async {
    ...
    // Fetch and return the webUrl so the UI can offer a "Go to file" button.
    final uri = Uri.parse(_itemPath(ref));
    final response = await _client.get(uri, headers: _authHeaders(accessToken));
    if (response.statusCode == 200) {
      final json = jsonDecode(response.body) as Map<String, dynamic>;
